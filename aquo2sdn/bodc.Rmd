...
kableExtra::kbl(sample_n(p01s, 10), caption = "Voorbeeld 10 elementen uit de tabel [P01 VOCABULARY - FACET SEARCH ON SEMANTIC COMPONENTS]. ")
    ...
    knitr::kable(sample_n(p01s, 10), format = "latex", booktabs = TRUE) %>%
...
Voor veel combinaties van AQUO (um-aquo) velden zijn in het verleden P01 parameternamen gezocht. Deze combinaties staan deels in de mappingP01 tabel die onderhouden wordt door AQUO. Daarnaast staan deze combinaties in andere lokale bestanden die ad hoc mappings hebben gefaciliteerd (bronnen: RWS, MARIS, Deltares). In dit project hebben we in eerste instantie geprobeerd om AQUO codes te koppelen aan P01 namen. 
In dit project worden de P01 namen opgesplitst in de semantische onderdelen (S-tabellen), en deze onderdelen zullen gematcht worden met de afzonderlijke AQUO elementen in de daarvoor geschikte tabellen Hiervoor is het nodig de verschillende "S" tabellen te koppelen aan AQUO tabellen. De waarden in deze tabellen worden dan gekoppeld met behulp van tabel \@ref(tab:p01s-tabel).
